...
class InMemoryBridge:
    """Deterministic Factorio-like bridge used only for V0 tests/demo."""
    ...
    def __init__(self, state: dict[str, Any] | None = None) -> None:
        ...
        self.accept_without_effect: set[str] = set()
    ...
    def _ack(self, action: str, accepted: bool, detail: str | None = None) -> Ack:
        return Ack(status="accepted" if accepted else "rejected", action_id=str(uuid.uuid4()), detail=detail)
    ...
    def act(self, action: str, params: Mapping[str, Any]) -> Ack:
        ...
        if action == "move_to":
            self.state["player"]["position"] = [float(params["x"]), float(params["y"])]; return self._ack(action, True)
        ...
        if action == "place_entity":
            name = str(params["name"]); pos = [float(params["x"]), float(params["y"])]
            for e in self.state.get("entities", []):
        ...
        if action == "transfer":
            item, count = str(params["item"]), int(params["count"]); x, y = float(params["x"]), float(params["y"])
            player_inv = self.state["player"].setdefault("inventory", {}); available = int(player_inv.get(item, 0)); moved = min(available, count)
            ...
            player_inv[item] = available - moved; entity.setdefault("inventory", {})[item] = int(entity.setdefault("inventory", {}).get(item, 0)) + moved
    ...
    def _entity_at(self, x: float, y: float) -> dict[str, Any] | None:
        for entity in self.state.get("entities", []):
            ...
            if ex == x and ey == y: return entity
        return None
