...
class IncidentManager:
    ...
    def create(self, reason: str, *, digest: Mapping[str, Any], task: Mapping[str, Any] | None, master_plan: Mapping[str, Any], actions: list[Mapping[str, Any]], failures: list[Mapping[str, Any]], nearby_state: Mapping[str, Any] | None, budget: Mapping[str, Any]) -> Path:
        ...
        path = self.root / f"{stamp}_{safe_reason}"
        suffix = 1
        while path.exists():
            suffix += 1
            path = self.root / f"{stamp}_{safe_reason}_{suffix}"
        path.mkdir(parents=True)
        self._write(path / "digest.json", digest)
        self._write(path / "task.json", task)
        self._write(path / "master_plan.json", master_plan)
        self._write(path / "actions.jsonl", actions)
        self._write(path / "failures.json", failures)
        self._write(path / "nearby_state.json", nearby_state)
        self._write(path / "budget.json", budget)
        return path
